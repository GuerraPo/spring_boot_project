...
import tlc.gab.ims.repositories.GabLicenseeAddressRepo;
import tlc.gab.ims.repositories.GabLicenseeContactRepo;
import tlc.gab.ims.repositories.GabLicenseeGameRecordRepo;
import tlc.gab.ims.repositories.GabLicenseeRepo;
import tlc.gab.ims.payloads.AddApplicant;

import java.util.List;

@Service
public class GabLicenseeGameRecordService {

    Logger log = LoggerFactory.getLogger(GabLicenseeGameRecordService.class);

    @Autowired
    private GabLicenseeGameRecordRepo gameRecordRepo;

    @Autowired
    private GabLicenseeRepo licenseeRepo;

    @Autowired
    private GabLicenseeAddressRepo addressRepo;

    @Autowired
    private GabLicenseeContactRepo contactRepo;

    public AddApplicant searchLicenseApplication(String licenseNumber, String name){
        AddApplicant applicationDetails = new AddApplicant();
        try{
            GabLicenseeGameRecord gameRecord = gameRecordRepo.searchByLicenseNumber(licenseNumber, name);
            if(gameRecord != null){
                String accountId = gameRecord.getAccountId();
                GabLicensee applicant = licenseeRepo.findByAccountId(accountId);
                List<GabLicenseeContact> contacts = contactRepo.findByAccountId(accountId);
                List<GabLicenseeAddress> addresses = addressRepo.findByAccountId(accountId);
